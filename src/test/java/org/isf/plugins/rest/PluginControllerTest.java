...
class PluginControllerTest {
	...
	private static final PluginDefinition SMART_DOC = new PluginDefinition(
		...
		new PluginConfiguration(List.of(
			new PluginPermission("admin", List.of(
				new PluginRoute("/documents", List.of("GET", "POST", "DELETE")))))));
	...
	void stubAuthenticationSupplier() {
		...
		lenient().when(authenticationSupplier.get())
			.thenAnswer(inv -> new AuthenticationContext(
				SecurityContextHolder.getContext().getAuthentication(), "admin"));
	}
	...
	void shouldReturn403WhenAccessIsDenied() throws Exception {
		...
		doThrow(new PluginAccessDeniedException("smart-doc", "alice"))
			.when(authorizationChecker).assertAccess(any(), any(), any());
