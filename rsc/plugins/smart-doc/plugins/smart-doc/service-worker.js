...
class CacheTimestampsModel {
  ...
  async getTimestamp(url) {
    ...
    return entry === null || entry === undefined ? undefined : entry.timestamp;
  }
...
registerRoute(({ url }) => url.pathname.endsWith(".ts") || url.pathname.endsWith(".tsx"), new NetworkOnly);
...
async function convertPostToGetCacheKey(request) {
  if (request.method !== "POST")
    return request;
  const body = await request.clone().text();
  const url = new URL(request.url);
  url.searchParams.set("postBody", body);
  return new Request(url.toString(), { method: "GET" });
}
var API_REQUEST_HANDLER = new NetworkFirst({
  cacheName: "api",
  plugins: [
    ...
    }),
    {
      async cacheKeyWillBeUsed({ request }) {
        return await convertPostToGetCacheKey(request);
      },
      async requestWillFetch({ request }) {
        const cache = await caches.open("api");
        const cacheRequest = await convertPostToGetCacheKey(request);
...
registerRoute(({ url, request }) => request.method === "GET" && /^\/api\//.test(url.pathname), API_REQUEST_HANDLER, "GET");
registerRoute(({ url, request }) => request.method === "POST" && /^\/api\/[^/]+\/search$/.test(url.pathname), API_REQUEST_HANDLER, "POST");
registerRoute(({ url }) => url.pathname.startsWith("/graphql"), new NetworkFirst({
  cacheName: "graphql"
}));
